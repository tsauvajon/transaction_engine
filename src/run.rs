use crate::{error_handler, input::parse, ledger::ledger, output};
use std::sync::mpsc;
...
pub fn run(
    ...
    let transaction_errors = ledger::build(transactions, account_tx);

    let error_handling_threads = error_handler::sink(input_errors, transaction_errors);
